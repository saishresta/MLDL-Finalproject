...
with torch.no_grad():   
    ...
    current_Te_loss = []
    val_set_len = 0
    val_dataset = HI_Dataset(data_path, 'test.csv')
   

    
    val_set_len += val_dataset.__len__()
    print("Testing set size: ", val_set_len ,"samples")
    ...
    for test_batch_E, test_batch_A, test_batch_y in valLoader:
        ...
        for i in range(test_batch_y.shape[0]):
            if predicted_class[i] == real_class[i]:
                ...
                # print(f"predicted_class:{predicted_class[i]}  real_class:{real_class[i]} ==> False")
                print("False")    
