...
# In[6]  Evaluaiton loop
    ...
    Tr_mean_Acc=np.round(correct/total, 4)*100
    elapsed_time = time.time() -t
    avg_tr_sum=avg_tr_sum+Tr_mean_Acc
    avg_te_sum=avg_te_sum+mean_acc
    avg_tr_acc=avg_tr_sum/(epoch+1)
    avg_te_acc=avg_te_sum/(epoch+1)
    
    
    
    print(f"Epoch:{epoch}  Duration:{np.round(elapsed_time,2)}   Train_Accuracy:{Tr_mean_Acc}  Test_Acc:{mean_acc}" )
    print(f"Tr_Avg_Acc={avg_tr_acc}  Te_Avg_Acc={avg_te_acc} \n")
